...
# Regularization --------------------------------------------------------------
def L2reg(mat):
    return T.sum((mat - 1) ** 2)
# -----------------------------------------------------------------------------


# Cost ------------------------------------------------------------------------
def reg_margincost(pos, neg, rmat, marge=1.0, regparam=0.0):
    out = neg - pos + marge
...
def mySimFn(fnsim, embeddings, leftop, rightop):
    """
    This function returns a Theano function to measure the similarity score
    for sparse matrices inputs.
    ...
    :param rightop: class for the 'right' operator.
    """
    embedding, relationMat, relationVec = parse_embeddings(embeddings)
    ...
    lhs = S.dot(embedding.E, inpl).T
    rhs = S.dot(embedding.E, inpr).T
    relmat = S.dot(relationMat.E, inpo).T
    relvec = S.dot(relationVec.E, inpo).T
    simi = fnsim(leftop((rhs-lhs), relmat), rightop(relvec, relvec))
...
def RankLeftFn(fnsim, embeddings, leftop, rightop,
    ...
    if not adding:
        ...
        return theano.function([inpr, inpo, inpladd, scal], [simi], on_unused_input='ignore')
...
def myRankRightFnIdx(fnsim, embeddings, leftop, rightop, subtensorspec=None):
    """
    This function returns a Theano function to measure the similarity score of
    all 'right' entities given couples of relation and 'left' entities (as
    index values).
    ...
    if subtensorspec is not None:
        ...
        rhs = embedding.E.T
    relmat = (relationMat.E[:, idxo]).reshape((1, relationMat.D))
    relvec = (relationVec.E[:, idxo]).reshape((1, relationVec.D))
    #tmp = 
    simi = fnsim(leftop(rhs-lhs, relmat), rightop(relvec, relvec))
    """
    Theano function inputs.
...
def myRankLeftFnIdx(fnsim, embeddings, leftop, rightop, subtensorspec=None):
    """
    This function returns a Theano function to measure the similarity score of
    all 'left' entities given couples of relation and 'right' entities (as
    index values).
...
def myRankRelFnIdx(fnsim, embeddings, leftop, rightop, subtensorspec=None):
    ...
    if subtensorspec is not None:
        # We compute the score only for a subset of entities
        relmat = (relationMat.E[:, :subtensorspec]).T
        relvec = (relationVec.E[:, :subtensorspec]).T
    else:
        relmat = relationMat.E.T
        relvec = relationVec.E.T
    simi = fnsim(leftop(rhs-lhs, relmat), rightop(relvec,relvec))
    """
    Theano function inputs.
    :input idxl: index value of the 'left' member.
...
def myTrainFn(fnsim, embeddings, leftop, rightop, marge=1.0, regparam=0.0):
    """
    This function returns a theano function to perform a training iteration,
    contrasting couples of positive and negative triplets. members are given
    as sparse matrices. for one positive triplet there is one negative
    ...
    lhsn = S.dot(embedding.E, inpln).T
    rhsn = S.dot(embedding.E, inprn).T
    relmatn = S.dot(relationMat.E, inpon).T
    relvecn = S.dot(relationVec.E, inpon).T
    ...
    if type(embeddings) == list:
        # If there are different embeddings for the relation member.
        gradients_embedding = T.grad(cost, relationMat.E)
        newE = relationMat.E - lrparams * gradients_embedding
        updates.update({relationMat.E: newE})
        gradients_embedding = T.grad(cost, relationVec.E)
        newE = relationVec.E - lrparams * gradients_embedding
        updates.update({relationVec.E: newE})
...
def myTrainFn1Member(fnsim, embeddings, leftop, rightop, marge=1.0, rel=True, regparam=0.0):
    """
    This function returns a theano function to perform a training iteration,
    contrasting positive and negative triplets. members are given as sparse
    matrices. For one positive triplet there are two or three (if rel == True)
    ...
    # Negative 'left' member
    similn = fnsim(leftop((rhs-lhs), relmat), rightop(relvec,relvec))

    # Negative 'right' member
    simirn = fnsim(leftop((rhs-lhsn), relmat), rightop(relvec,relvec))

#    costl, outl = margincost(simi, similn, marge)
#    costr, outr = margincost(simi, simirn, marge)

    costl, outl = reg_margincost(simi, similn, relmat, marge, regparam)
    costr, outr = reg_margincost(simi, simirn, relmat, marge, regparam)
    cost = costl + costr
    out = T.concatenate([outl, outr])
    # List of inputs of the function
    list_in = [lrembeddings, lrparams,
...
#    list_in = [lrembeddings, lrparams,
#            inpl, inpr, inpo, inpln, inprn, slhs, srhs, srelvec, srelmat, slhsn, srhsn]
    ...
    if rel:
        ...
        simion = fnsim(leftop(lhs, relln), rightop(rhs, relrn))
#        costo, outo = margincost(simi, simion, marge)
        costo, outo = reg_margincost(simi, simion, relmat, marge, regparam)
        cost += costo
        out = T.concatenate([out, outo])
        list_in += [inpon]
...
def myTrainFn1Member_relonly(fnsim, embeddings, leftop, rightop, marge=1.0, regparam=0.0):
    """
    This function returns a theano function to perform a training iteration,
    contrasting positive and negative triplets. members are given as sparse
    matrices. For one positive triplet there are two or three (if rel == True)
...
#    costr, outr = margincost(simi, simirn, marge)
    ...
    # If rel is True, we also consider a negative relation member
    inpon = S.csr_matrix()
    relln = S.dot(relationMat.E, inpon).T
    relrn = S.dot(relationVec.E, inpon).T
    simion = fnsim(leftop((rhs-lhsn), relmat), rightop(relvec,relvec))
#        costo, outo = margincost(simi, simion, marge)
    costo, outo = reg_margincost(simi, simion, relmat, marge, regparam)
    cost = costo
    out = T.concatenate([outo])
    list_in += [inpon]
